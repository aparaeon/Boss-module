...
public enum ClickType {
	...
	public static ClickType toClickType(net.minecraft.world.inventory.ClickType action, int button, int slot) {
		return switch (action) {
			case PICKUP -> button == 0 ? MOUSE_LEFT : MOUSE_RIGHT;
			case QUICK_MOVE -> button == 0 ? MOUSE_LEFT_SHIFT : MOUSE_RIGHT_SHIFT;
			case SWAP -> {
				if (button >= 0 && button < 9) {
					yield values()[button + 4];
				} else if (button == 40) {
					yield OFFHAND_SWAP;
				} else {
					yield UNKNOWN;
			...
			case CLONE -> MOUSE_MIDDLE;
			case THROW ->
					slot == -999 ? (button == 0 ? MOUSE_LEFT_OUTSIDE : MOUSE_RIGHT_OUTSIDE) : (button == 0 ? DROP : CTRL_DROP);
			case QUICK_CRAFT -> switch (button) {
				case 0 -> MOUSE_LEFT_DRAG_START;
				case 1 -> MOUSE_LEFT_DRAG_ADD;
				case 2 -> MOUSE_LEFT_DRAG_END;
				case 4 -> MOUSE_RIGHT_DRAG_START;
				case 5 -> MOUSE_RIGHT_DRAG_ADD;
				case 6 -> MOUSE_RIGHT_DRAG_END;
				case 8 -> MOUSE_MIDDLE_DRAG_START;
				case 9 -> MOUSE_MIDDLE_DRAG_ADD;
				case 10 -> MOUSE_MIDDLE_DRAG_END;
				case 3, 7 -> UNKNOWN;
				default -> UNKNOWN;
			};
			case PICKUP_ALL -> MOUSE_DOUBLE_CLICK;
		};
	}
}
