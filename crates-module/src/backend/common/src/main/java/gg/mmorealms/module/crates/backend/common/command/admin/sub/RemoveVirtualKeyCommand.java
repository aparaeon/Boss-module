...
@Command(aliases = {"remove_virtual_key"}, parent = AdminCommand.class, arguments = {"player", "amount", "type"})
public class RemoveVirtualKeyCommand extends BackendCommand {

	private @Inject CratesConfig config;
	private @Inject MinecraftServer server;

	public RemoveVirtualKeyCommand(CommonCommandManager commandManager) {
	...
	protected void executeCommon(@NotNull CommandSource sender, @NotNull List<String> arguments) {
		...
		if (amount <= 0) {
			sendMessage(sender, "<red>Please use \"/crates admin give_virtual_key\" instead if you want to give keys");
			return;
		}
	...
	private void execute(CommandSource sender, IUser target, int amount, String type) {
		...
		crateKeys.removeKeys(type, amount);
		sendMessage(sender, new MessageBuilder("You have removed {amount} {type} keys to {player}")
				.parse("amount", amount)
				.parse("type", type)
