...
@Command(aliases = {"sync"}, onlyFor = Command.OnlyFor.PLAYERS, parent = DiscordCommand.class)
public class SyncSubCommand extends VelocityCommand {

	public @Inject DiscordLinkConfig config;

	public SyncSubCommand(CommonCommandManager commandManager) {
	...
	protected void executePlayer(Player player, List<String> arguments) {
		...
		DiscordLinkVelocityModule.instance().getDiscordLinkerManager().syncRanks(user);
	}
}
