...
@Command(aliases = {"givepoint"}, parent = ChatGamesAdminCommand.class, arguments = {"ign", "amount"})
public class GivePointCommand extends VelocityCommand {

	private @Inject VelocityMiniMessageManager miniMessageManager;

	public GivePointCommand(CommonCommandManager commandManager) {
	...
	protected void executeCommon(CommandSource sender, List<String> arguments) {
		...
		ScheduleUtils.runTaskAsync(() -> {
			...
			Logger.info("[ChatGames] Added " + finalAmount + " point(s) for " + ign + " via givepoint command (executor: " + senderName + ")");

			sender.sendMessage(miniMessageManager.parse(
				ChatGamesVelocityModule.instance().getConfig().lang.adminPointGiven
					.parse("ign", ign)
