...
@Command(aliases = {"chatgames", "cg"})
public class ChatGamesCommand extends VelocityCommand {

	private @Inject VelocityMiniMessageManager miniMessageManager;

	public ChatGamesCommand(CommonCommandManager commandManager) {
	...
	protected void executeCommon(CommandSource sender, List<String> arguments) {
		sender.sendMessage(miniMessageManager.parse(
			ChatGamesVelocityModule.instance().getConfig().lang.help.parse()
		));
	}
